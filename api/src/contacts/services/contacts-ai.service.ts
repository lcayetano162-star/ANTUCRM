...
@Injectable()
export class ContactsAIService {
  private readonly logger = new Logger(ContactsAIService.name);
  ...
  async generateDashboardInsights(
    ...
    try {
      ...
      // Obtener datos relevantes para insights
      const [
        contactsWithoutActivity,
        contactsWithPendingTasks,
...
Los insights deben ser específicos, accionables y relevantes para vendedores.`;
      ...
      const insights = await this.gemini.generateStructured<ContactAIInsight[]>(prompt, {
        type: 'array',
        items: {
          type: 'object',
          properties: {
...
[{
  ...
  "action": { "label": "string" }
      ...
      return await this.gemini.generateStructured<ContactAIInsight[]>(prompt, {
        type: 'array',
        items: {
          type: 'object',
          properties: {
...
Genera un scoring en formato JSON:
{
  ...
  "explanation": "string (explicación breve del score)"
      ...
      return await this.gemini.generateStructured<ContactScoringResult>(prompt, {
        score: 'number',
        factors: 'array',
        explanation: 'string',
      });
...
Genera una recomendación en formato JSON:
{
  ...
  "suggestedMessage": "string (mensaje sugerido)"
      ...
      return await this.gemini.generateStructured<NextBestAction>(prompt, {
        action: 'string',
        priority: 'string',
        reason: 'string',
  ...
  async getContactHealthScore(
    tenantId: string,
    contactId: string,
  ): Promise<ContactHealthScore> {
    try {
      const contact = await this.prisma.contact.findFirst({
        where: { id: contactId, tenantId },
        include: {
      ...
      // Frecuencia de actividad (actividades por semana en los últimos 30 días)
      const activitiesLast30Days = activities.filter(
        a => (now - a.createdAt.getTime()) / (1000 * 60 * 60 * 24) <= 30,
      ).length;
      const activityFrequency = Math.min(activitiesLast30Days / 4, 5) * 20; // 0-100

      // Engagement basado en diversidad de tipos de actividad
      const activityTypes = new Set(activities.map(a => a.type)).size;
      const engagement = Math.min(activityTypes / 5, 1) * 100;

      // Tiempo de respuesta (simulado basado en frecuencia)
      const responseTime = Math.max(0, 100 - daysSinceLastActivity * 5);

      // Última actividad score
      ...
      const activitiesPrevious30Days = activities.filter(
        a => {
  ...
  private getDefaultInsights(): ContactAIInsight[] {
    return [
      {
        id: 'default-1',
        type: 'ENGAGEMENT',
  ...
  private getDefaultScoring(): ContactScoringResult {
    return {
      score: 50,
      factors: [
  ...
  private getDefaultNextAction(): NextBestAction {
    return {
      action: 'EMAIL',
      priority: 'MEDIUM',
